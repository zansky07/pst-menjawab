	...
	<main class="flex-grow mb-24">
		<div class="container mx-auto p-6"> <?php if (session()->getFlashdata('message')): ?> <p class="text-green-500"> <?= session()->getFlashdata('message') ?> </p> <?php endif; ?>
			...
			<div class="flex justify-end mb-4">
				<button id="filterBtn" class="bg-oranye-3 hover:bg-orange-700 text-white font-bold py-2 px-5 rounded-lg">Filter</button>
			</div>
			<!-- Filter Modal -->
			...
			<div class="bg-white shadow-md rounded-lg overflow-hidden">
				<div class="table-container">
					<table class="min-w-full bg-white">
						<thead class="bg-oranye-2 text-white">
							<tr>
										...
										<td class="py-2 px-4">
											<div class="flex justify-around">
												<a href="/admin/consultation/detail/
																<?= $request['id'] ?>" class="bg-green-500 text-white py-1 px-1 rounded-lg w-full text-center mx-1 text-sm transition duration-300 hover:bg-green-600">Detail </a>
												<a onclick="confirmDelete('/admin/consultation/delete/<?= $request['id'] ?>')"
													class="bg-red-500 text-white py-1 px-1 rounded-lg w-full text-center mx-1 text-sm transition duration-300 hover:bg-red-600" onclick="return confirm('Apakah Anda yakin ingin menghapus?')">Hapus </a>
											</div>
										</td>
